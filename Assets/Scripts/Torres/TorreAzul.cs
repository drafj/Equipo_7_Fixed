...
public class TorreAzul : MonoBehaviour
{
  // public Text textoReparacion;
   ...
   public GameObject torreAzulReparada;
    ...
    void Update()
    {
        if (GameManager.instance.activadorToolKits[0] && sePuedeReparar == true )
        {
            if (reparando == false )
            {
   ...
   {
       if (other.gameObject.GetComponent<Player>() && GameManager.instance.activadorToolKits[0] && Input.GetKeyDown(KeyCode.F) && !GameManager.instance.armaSacada)
        {
            reparando = false;
